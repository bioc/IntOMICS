...
using `ggraph_weighted_net` (node size and label size can be modified) 
and `legend_custom_ggplot` functions and arrange them into one figure:

```{r echo=TRUE, include=FALSE, eval=FALSE}

rn <- ggraph_weighted_net(net = weighted_net_res)
...
# ggraph_weighted_net scale_color_hue(labels = c("new", "PK"))
library(cowplot)
##### plot_grid(rn, leg, ncol = 1, rel_heights = c(3, 1))
# weighted_net_legend(net = weighted_net_res)
#grobs <- ggplotGrob(rn)$grobs
#legend <- grobs[[which(sapply(grobs, function(x) x$name) == "guide-box")]]
# build grid without legends
#pgrid <- plot_grid(rn, leg, ncol = 1, rel_heights = c(3, 1), rel_widths = c(7,7))
# add legend
