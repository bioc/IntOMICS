...
coad_ma <- subsetByColData(coad_ma, coad_ma$MSI_status == "MSI-H" | is.na(coad_ma$MSI_status))
# choose first 50 samples for illustration
coad_ma <- subsetByColData(coad_ma, c(rep(TRUE,50),rep(FALSE,ncol(coad_ma[[1]])-50)))
...
coad_toy <- coad_ma[rowselect, , ]

# choose only primary tumor samples
ge <- t(assay(coad_toy[[2]]))
cnv_char <- t(assay(coad_toy[[1]]))
cnv <- apply(cnv_char,2,as.numeric)
rownames(cnv) <- rownames(cnv_char)
meth <- t(as.matrix(assay(coad_toy[[3]])))
...
BN_mod_res <- BN_module(burn_in = 10000, 
                        thin = 500, 
...
For the illustration, we have used only short burn-in period. We recommend to use much longer burn-in period, such as 100000 and check trace plots of the MCMC simulation.
The same goes for minseglen parameter (for further details see (Pacinkova \& Popovici, 2022)).
...
Pacinkova, A. \& Popovici, V. (2022). Using Empirical Biological Knowledge to Infer Regulatory Networks From Multi-omics Data. BMC Bioinformatics, 23.
