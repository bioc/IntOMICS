...
This tutorial will show you how to use the IntOMICS package with a toy example. The example dataset consisting of processed gene expression, DNA methylation (Illumina Infinium HumanMethylation450 BeadChip) and copy number variation is from the TCGA data portal (https://portal.gdc.cancer.gov/): primary colon cancer samples (COAD) with microsatellite instability (MSI). However, the approach is not limited to any phenotype. We choose the set of 8 genes from the KEGG Colorectal cancer pathway (https://www.genome.jp/pathway/hsa05210).
...
library(IntOMICS)
data(list=c("BN_mod_res", "PK", "TFtarg_mat", "annot", "gene_annot", "layers_def"), package="IntOMICS")
...
coad_SE <- curatedTCGAData("COAD", c("RNASeq2GeneNorm_illuminahiseq","Methylation_methyl450","GISTIC_AllByGene"), version = "2.0.1", dry.run = FALSE)

coad_GE_CNV <- coad_SE[,,c("COAD_RNASeq2GeneNorm_illuminahiseq-20160128","COAD_GISTIC_AllByGene-20160128")]
rownames(coad_GE_CNV[["COAD_GISTIC_AllByGene-20160128"]]) <- rowData(coad_GE_CNV[["COAD_GISTIC_AllByGene-20160128"]])[["Gene.Symbol"]]

# choose only MSI-H samples
coad_GE_CNV_clinical <- longFormat(coad_GE_CNV,colDataCols = c("MSI_status","patient.samples.sample.sample_type"))
coad_GE_CNV_clinical_MSI <- coad_GE_CNV_clinical[which(coad_GE_CNV_clinical$MSI_status=="MSI-H"),]

coad_GE_CNV <- coad_GE_CNV[gene_annot$gene_symbol, unique(coad_GE_CNV_clinical_MSI$primary),]
coad_METH <- coad_SE[unlist(annot),unique(coad_GE_CNV_clinical_MSI$primary),"COAD_Methylation_methyl450-20160128"]
coad_GE_CNV_METH <- c(coad_GE_CNV, coad_METH)
coad_GE_CNV_METH <- coad_GE_CNV_METH[,complete.cases(coad_GE_CNV_METH),]

# choose only primary tumor samples
ge <- t(assay(coad_GE_CNV_METH[[1]])[,colnames(assay(coad_GE_CNV_METH[[1]]))[regexpr("-01A-",colnames(assay(coad_GE_CNV_METH[[1]])))>0]])
cnv_char <- t(assay(coad_GE_CNV_METH[[2]])[,colnames(assay(coad_GE_CNV_METH[[2]]))[regexpr("-01A-",colnames(assay(coad_GE_CNV_METH[[2]])))>0]])
cnv <- apply(cnv_char,2,as.numeric)
rownames(cnv) <- rownames(cnv_char)
meth <- t(as.matrix((assay(coad_GE_CNV_METH[[3]])[,colnames(assay(coad_GE_CNV_METH[[3]]))[regexpr("-01A-",colnames(assay(coad_GE_CNV_METH[[3]])))>0]])))

omics <- list(ge=ge, cnv=cnv, meth=meth)
omics <- lapply(omics,FUN=function(mat) {rownames(mat) <- substr(rownames(mat),1,12); mat})

```

Available omics data are saved in a named list `omics` including gene expression (GE) of 8 genes + copy number variation (CNV) of 8 genes + beta value of 9 DNA methylation (METH) probes:
...
```
These values correspond to normalised RNA-seq data (in this case transcripts per million (TPM)). 
However, the user is not limited to this platform. Another assay, such as microarray data, can be used.
...
```
`annot` is a named list. Each component of the list is a character vector and corresponds to probe IDs associated with a given gene. Names of the annot must be again in the format EID:XXXX.  
...
```
`gene_annot` is Gene ID conversion table with "entrezID" and "gene_symbol" column names. Entrez IDs are used in for regulatory network inference, gene symbols are used for the final regulatory network visualisation.    
...
We can use linear regression to filter irrelevant DNA methylation probes. In such a case, we set the parameter "lm_METH = TRUE".
...
Once it is created, we can run the `trace_plots` function:  
...
1. beta_values.jpeg: trace plot of beta values (we want to explore the sample space many times and avoid flat bits - the chain stays in the same state for too long; in this case, beta value fluctuates around single value, so the Markov chain is mixing well).  
<center>
![](figures/MSI/beta_values.jpeg){width=50%}
</center>

2. post_prob_edges.jpeg: consistency of edges posterior probabilities in two independent MCMC simulations (scatter plot of the edge weights confidence using two independent MCMC runs; the convergence is determined by the spread of the points around the y=x line; in this case, the edge weights seems to be consistent in two independent simulations).  
<center>
![](figures/MSI/post_prob_edges.jpeg){width=50%}
</center>

3. convergence_RMS.jpeg: the c<sub>rms</sub> strength for the convergence evaluation (summarizes the spread of the points around the line y=x in post_prob_edges.jpeg, for details see (Agostinho et al., 2015) and (Pacinkova \& Popovici, 2022)).  
<center>
![](figures/MSI/convergence_RMS.jpeg){width=50%}
</center>
